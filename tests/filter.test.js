...
test('Expecting filter() to return empty array for null values', () => {
    expect(filter(null, null)).toStrictEqual([[]]);
})
test('Expecting filter() to return empty array for undefined values', () => {
    expect(filter(undefined, undefined)).toStrictEqual([[]]);
})